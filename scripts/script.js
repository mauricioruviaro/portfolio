...
if (contentObject[window.location.hash]) {
    ...
    const element = addClassFromHashAndReturnElement('selected-a-tag', contentObject[window.location.hash], Array.from(navItens))
    addIcon(element)
} else {
    removeClassFromList('selected-a-tag', Array.from(navItens))
    const element = addClassFromHashAndReturnElement('selected-a-tag', 'Home', Array.from(navItens))
    addIcon(element)
}
...
document.addEventListener('click', ({ target }) => {
    if (contentObject[target.hash]) {
        removeClassFromList('selected-a-tag', Array.from(navItens))
        addClassFromHashAndReturnElement('selected-a-tag', contentObject[target.hash], Array.from(navItens))
        addIcon(target)
    }
})
